...
const addprojectinfo = async (req, res) => {
  try {
    ...
    const newProject = new Project({
      ...
      queansdescription,
      projectlogo,
      brochurepdfUrl, // Store the brochure URL in the project document
    });
...
const updateprojectdata = asyncHandler(async (req, res) => {
  ...
  const { field, index, newData, file } = req.body;
  ...
  try {
    ...
    if (field === "projectname" || field === "projectlocation") {
    ...
    } else if (field === "brochurepdfUrl") {
      if (file) {
        const brochureFile = file;
        const uploadParams = {
          Key: brochureFile.originalname,
          Body: brochureFile.buffer,
        };
        const uploadResult = await uploadFileToS3(uploadParams);
        brochurepdfUrl = uploadResult.Location;
        updateQuery[`${field}.${index}`] = brochurepdfUrl;
      }
    } else {
